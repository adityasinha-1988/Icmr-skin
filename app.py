...
# UI Toggle for Bio-Team
run_mode = st.radio(
    "Select Optimization Engine:", 
    ["⚡ Fast Mode (XGBoost Surrogate + Clustering - Instant)", "🔬 Deep Mode (Includes Bayesian GP - Takes ~30 seconds)"]
)
...
if st.button("Generate Next 5 Optimal Formulations"):
    with st.spinner(f"Running {run_mode.split(' ')[1]}..."):
        try:
            ...
            # 1. Generate Candidate Pool (10,000 points)
            n_samples = 10000
            ...
            # 2. Apply Hard Chemistry Constraints (Physics Filter)
            ...
            if len(valid_cands) < 5:
                ...
                # 3. Direct Surrogate Efficacy (Always Runs)
                ...
                # 4. Deep Mode: Bayesian GP Addition (Only if selected)
                if "Deep" in run_mode:
                    ...
                    hist_df = pd.read_csv('dummy_lab_data.csv') # MAKE SURE THIS FILE EXISTS IN REPO
                    ...
                    # Strictly limit to 300 points to prevent 15 min hang
                    ...
                    with torch.no_grad():
                        ...
                        ei_scores = sigma * (u * normal.cdf(u) + normal.log_prob(u).exp())
                    
                    # Normalize and Combine for the first 300 points
                    ...
                    # Overwrite the first 300 scores with the combined consensus score
                    final_scores[:300] = (ei_norm * 0.5) + (eff_norm * 0.5)

                # 5. Consensus Node (Diversity Clustering via K-Means)
                # Take top 50 based on final_scores
                ...
                st.success(f"Optimization Complete! ({run_mode.split(' ')[1]}). Here are 5 strictly diverse, safe O/W formulations:")
